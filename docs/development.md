...
To develop bitcore-node-finite:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-finite.git
...
We now will setup symlinks in `bitcore-node-finite` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-finite
ln -s ~/bitcoind-rpc-finite
```
...
cd devnode
mkdir node_modules
touch bitcore-node-finite.json
touch package.json
```

Edit `bitcore-node-finite.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-finite/bin/bitcore-node-finite start
```